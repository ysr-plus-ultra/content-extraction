...
class AbstractExtractor(ABC):
    ...
    def extract_content(self, html: str) -> str:
        ...
        # print(bs.prettify())
        # bs = bs.find_all(KG_MARK, 1)
        output_dirty = bs.get_text(strip=True)
        # print(len(bs.children))
    ...
    def preprocess_dom(self, web_element: Tag):
        ...
        for _tag in self.removables:
            remove += web_element.find_all(_tag)
...
class Extractor(AbstractExtractor):
    ...
    def compute_text_density(self, web_element: Tag, ratio: float):
        ...
        if char_num == 0:
            ...
            assert un_linkchar_num >= 0
            assert (tag_num - linktag_num) >= 0
            if tag_num == 0:
                tag_num = 1
            if linkchar_num == 0:
                linkchar_num = 1
        ...
        if isinstance(web_element, Tag) and web_element.name != None:
            ...
            for child in web_element.children:
                if isinstance(child, Tag) and child.name != None:
                    ...
                    index = content.find(child_content, from_)
                    if index > -1:
                        length = index - from_
                        if length > 0:
